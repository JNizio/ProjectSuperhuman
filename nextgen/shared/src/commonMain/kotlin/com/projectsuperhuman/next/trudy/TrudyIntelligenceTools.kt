...
class TrudyIntelligenceToolService(private val library: TrudyPersonalEvidenceLibrary, private val source: TrudyPersonalEvidenceSource, private val experiments: TrudyExperimentEngine = TrudyExperimentEngine()) : TrudyToolExecutor {
    ...
    override suspend fun execute(operation: TrudyToolOperation): TrudyToolResult = try {
        when(operation) {
            ...
            is GetLaggedAssociation -> { require(operation.lagMs in 0..MAX_LAG_MS); AssociationToolResult(operation,library.association(operation.leftDomain,operation.leftMetricId,operation.rightDomain,operation.rightMetricId,operation.method,operation.lagMs,operation.alignmentWindowMs)) }
...
class CompositeTrudyToolExecutor(
    ...
    override suspend fun execute(operation: TrudyToolOperation): TrudyToolResult = when (operation) {
        ...
        is EvaluateExperiment -> intelligenceExecutor.execute(operation)

...
FORBIDDEN_NAME_FRAGMENTS = {"android_dump", "device_dump", "raw_database", "health_export", "conversation_export"}
SECRET_PATTERNS = [
    re.compile(r"(?i)(api[_-]?key|access[_-]?token|secret|bearer)\s*[:=]\s*[^\s,;]{8,}"),
...
def validate_training_path(path: str | Path) -> None:
    ...
    if p.suffix.lower() in FORBIDDEN_SUFFIXES:
        raise PrivacyViolation(f"forbidden training input type: {p.suffix}")
    if any(fragment in low_name for fragment in FORBIDDEN_NAME_FRAGMENTS):
        raise PrivacyViolation(f"forbidden training input name: {p.name}")
...
def scan_sensitive(value: Any, path: str = "root") -> list[str]:
    ...
    if isinstance(value, dict):
        ...
        for index, child in enumerate(value): findings.extend(scan_sensitive(child, f"{path}[{index}]"))

...
/**
 * Hybrid BP monitor reader.
 *
 * OpenCV handles the hard photographic part (glare/contrast/display localisation,
 * threshold variants and small deskew rotations). The existing deterministic
 * seven-segment reader then recognises the actual digits. Multiple independent
 * passes vote for the best physiologically-plausible SYS/DIA/PULSE triple.
 */
final class HybridBloodPressureDecoder {
    ...
    static Result decode(Bitmap src){
        ...
        try{
            ...
            // Resize very large camera images to a predictable working scale.
            double scale=Math.min(1.0,1600.0/Math.max(gray.cols(),gray.rows()));
            ...
            org.opencv.imgproc.CLAHE clahe=Imgproc.createCLAHE(2.4,new Size(8,8));
            clahe.apply(enhanced,enhanced);

            List<Rect> rois=findDisplayCandidates(enhanced);
            // Always include broad central candidates so unusual monitor housings still work.
            ...
            for(Rect r:rois){
                ...
                evaluateVariant(roi,votes,"roi"+roiIndex+"-gray");
                ...
                evaluateVariant(adaptive,votes,"roi"+roiIndex+"-adaptive");

                Mat otsu=new Mat();
                Imgproc.threshold(roi,otsu,0,255,Imgproc.THRESH_BINARY+Imgproc.THRESH_OTSU);
                evaluateVariant(otsu,votes,"roi"+roiIndex+"-otsu");

                // Mild deskew ensemble. Phone photos are commonly a few degrees off-axis.
                for(double angle:new double[]{-7.0,-3.5,3.5,7.0}){
                    Mat rot=rotate(roi,angle);
                    evaluateVariant(rot,votes,"roi"+roiIndex+"-rot"+angle);
                    rot.release();
                }
                adaptive.release();otsu.release();roi.release();
            }
    ...
    private static void evaluateVariant(Mat m,List<Vote> votes,String source){
        ...
        Mat use=m;
        Mat enlarged=null;
        if(Math.max(m.cols(),m.rows())<900){
            enlarged=new Mat();Imgproc.resize(m,enlarged,new Size(m.cols()*1.55,m.rows()*1.55),0,0,Imgproc.INTER_CUBIC);use=enlarged;
        }
        Bitmap b=null;
        try{
            Mat rgba=new Mat();
            if(use.channels()==1) Imgproc.cvtColor(use,rgba,Imgproc.COLOR_GRAY2RGBA); else use.copyTo(rgba);
            b=Bitmap.createBitmap(rgba.cols(),rgba.rows(),Bitmap.Config.ARGB_8888);Utils.matToBitmap(rgba,b);rgba.release();
    ...
    private static void addVote(List<Vote> votes,int s,int d,int p,double score,String source){
        for(Vote v:votes){
            // Treat one-digit jitter as the same consensus cluster.
    ...
    private static Result chooseConsensus(List<Vote> votes){
        ...
        for(Vote v:votes){
            ...
            double rank=avg + v.count*48.0;
            if(v.s>=90&&v.s<=180)rank+=12;
            if(v.d>=50&&v.d<=110)rank+=12;
            if(v.p>=45&&v.p<=130)rank+=8;
            int pp=v.s-v.d;if(pp>=25&&pp<=90)rank+=9;
            if(best==null||rank>bestRank){best=v;bestRank=rank;}
        }
        double conf=Math.min(.995,.48+Math.min(5,best.count)*.09+Math.min(.16,bestRank/1200.0));
        return new Result(best.s,best.d,best.p,conf,"opencv_7seg_consensus","votes="+best.count+" source="+best.source+" rank="+Math.round(bestRank));
    }
    ...
    private static List<Rect> findDisplayCandidates(Mat gray){
        ...
        for(MatOfPoint c:contours){
            Rect r=Imgproc.boundingRect(c);double a=r.area();double ar=r.width/(double)Math.max(1,r.height);
            if(a>area*.055&&a<area*.82&&ar>.45&&ar<2.2&&r.width>150&&r.height>150) out.add(expand(r,gray.cols(),gray.rows(),.06));
            c.release();
        }
        ...
        // Remove near-duplicate overlapping rectangles.
        List<Rect> unique=new ArrayList<>();
        for(Rect r:out){boolean dup=false;for(Rect q:unique)if(iou(r,q)>.72){dup=true;break;}if(!dup)unique.add(r);if(unique.size()>=5)break;}
        return unique;
    }

    private static void addFallbackRois(List<Rect> rs,int w,int h){
        float[][] f={{.12f,.06f,.76f,.86f},{.20f,.10f,.62f,.78f},{.26f,.12f,.52f,.74f}};
        for(float[] a:f){Rect r=new Rect((int)(w*a[0]),(int)(h*a[1]),Math.max(1,(int)(w*a[2])),Math.max(1,(int)(h*a[3])));if(r.x+r.width>w)r.width=w-r.x;if(r.y+r.height>h)r.height=h-r.y;rs.add(r);}
    }

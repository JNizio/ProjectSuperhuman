...
SCHEMA_VERSION = "1.0"
DATASET_VERSION = "trudy-starter-1"
TOOL_CONTRACT_VERSION = "integration-2"
MODEL_POLICY_VERSION = "integration-2"
...
SENSITIVE_KEYS = {"user_id", "device_id", "android_id", "advertising_id", "database_path", "sql", "api_key", "token", "email", "phone"}
    ...
    if isinstance(obj, dict):
        for key, value in obj.items():
            ...
            if low in FORBIDDEN_KEYS:
                errors.append(f"forbidden chain-of-thought field: {here}")
            if low in SENSITIVE_KEYS:
                errors.append(f"sensitive field forbidden in training data: {here}")
            errors.extend(_walk(value, here))
    elif isinstance(obj, (list, tuple)):
        for i, value in enumerate(obj):
            errors.extend(_walk(value, f"{path}[{i}]"))
    return errors
...
def validate_example_dict(data: dict[str, Any]) -> list[str]:
    ...
    if data.get("task_category") not in TASK_CATEGORIES:
        errors.append(f"invalid task_category: {data.get('task_category')}")
    ...
    for result in data.get("tool_results", []):
        for evidence in result.get("evidence", []):
            if evidence.get("evidence_id"):
                supplied.add(evidence["evidence_id"])
    invalid = sorted(ref for ref in refs if ref and ref not in supplied)
    if invalid:
        errors.append("invalid evidence reference(s): " + ", ".join(invalid))
    return errors

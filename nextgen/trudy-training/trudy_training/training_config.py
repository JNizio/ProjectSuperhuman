...
PRECISIONS={"fp32","fp16","bf16"}; QUANTIZATION_MODES={"none","4bit","8bit"}
SECRET_KEYS={"api_key","token","access_token","refresh_token","secret","password","credential"}
...
class FineTuneConfig:
    ...
    def from_dict(cls,data:dict[str,Any])->"FineTuneConfig":
        low={str(k).lower() for k in data}
        forbidden=sorted(low&SECRET_KEYS)
        if forbidden: raise ValueError("secrets are forbidden in training config: "+", ".join(forbidden))
        allowed={f.name for f in fields(cls)}; unknown=sorted(set(data)-allowed)
        if unknown: raise ValueError("unknown training config field(s): "+", ".join(unknown))
        values=dict(data)
        if "target_modules" in values: values["target_modules"]=tuple(values["target_modules"])
        cfg=cls(**values); cfg.validate(); return cfg

    @classmethod
    def load(cls,path:str|Path)->"FineTuneConfig": return cls.from_dict(json.loads(Path(path).read_text(encoding="utf-8")))
    ...
    def validate(self)->None:
        ...
        if not 0<=self.lora_dropout<1: errors.append("lora_dropout must be in [0,1)")
        ...
        if not 0<=self.warmup_ratio<1: errors.append("warmup_ratio must be in [0,1)")
        ...
        if self.save_steps<=0 or self.logging_steps<=0: errors.append("save_steps and logging_steps must be > 0")
    ...
    def config_hash(self)->str: return sha256(self.canonical_json().encode()).hexdigest()

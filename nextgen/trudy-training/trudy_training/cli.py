...
from .audit import audit_dataset,near_duplicates,overlap
...
from .generator import GENERATOR_VERSION,generate_synthetic
from .gold import gold_examples
from .gold_hardening import hardening_gold_examples
...
from .promotion import PromotionPolicy,compare_and_gate,evaluate_predictions,markdown_report
from .provenance import build_manifest as build_dataset_manifest,quality_metrics
from .schema import DATASET_VERSION,MODEL_POLICY_VERSION,SCHEMA_VERSION,TOOL_CONTRACT_VERSION,validate_example_dict
from .split import deterministic_split
from .training_config import FineTuneConfig


def combined(seed:int=17): return gold_examples()+hardening_gold_examples()+generate_synthetic(seed)
...
def _predictions(path:str,strict:bool=False)->dict[str,dict]:
    rows=read_jsonl(path); out={}
    from .prediction import validate_prediction
    for row in rows:
        p=row.get('prediction',row); errs=validate_prediction(p) if strict else []
        if errs: raise ValueError(f"invalid candidate prediction {p.get('example_id','?')}: {errs}")
        out[p['example_id']]=p
    return out
...
def main(argv=None):
    ...
    au=sub.add_parser('audit'); au.add_argument('path'); au.add_argument('out_json')
    pv=sub.add_parser('provenance'); pv.add_argument('path'); pv.add_argument('out_json'); pv.add_argument('--seed',type=int,default=17)
    ...
    pr=sub.add_parser('promote'); pr.add_argument('examples'); pr.add_argument('baseline_predictions'); pr.add_argument('candidate_predictions'); pr.add_argument('policy'); pr.add_argument('out_json'); pr.add_argument('--markdown')
    ...
    try:
        if args.cmd=='validate': examples=_validated(args.path); print(json.dumps({'valid':True,'examples':len(examples)})); return 0
        ...
        if args.cmd=='split':
            examples=_validated(args.path); ensure_excluded_from_training(examples); train,val=deterministic_split(examples,args.validation_fraction); d=Path(args.out_dir); d.mkdir(parents=True,exist_ok=True); write_jsonl(d/'train.jsonl',train); write_jsonl(d/'validation.jsonl',val); print(json.dumps({'train':len(train),'validation':len(val),'prompt_overlap':len(overlap(train,val))})); return 0
        if args.cmd=='export': examples=_validated(args.path); Path(args.out).write_text(export_lines(examples,args.format,training=args.training),encoding='utf-8'); print(json.dumps({'examples':len(examples),'format':args.format})); return 0
        if args.cmd=='audit':
            examples=_validated(args.path); data=audit_dataset(examples); data['quality_metrics']=quality_metrics(examples,data['near_duplicate_count']); Path(args.out_json).write_text(json.dumps(data,indent=2,sort_keys=True)+'\n'); print(json.dumps(data,sort_keys=True)); return 0 if not data['duplicate_ids'] and not data['exact_prompt_duplicate_groups'] and not data['repeated_evidence_ids'] else 4
        if args.cmd=='provenance':
            examples=_validated(args.path); data=build_dataset_manifest(examples,schema_version=SCHEMA_VERSION,tool_contract_version=TOOL_CONTRACT_VERSION,policy_version=MODEL_POLICY_VERSION,generator_version=GENERATOR_VERSION,seed=args.seed,dataset_version=DATASET_VERSION); Path(args.out_json).write_text(json.dumps(data,indent=2,sort_keys=True)+'\n'); print(json.dumps(data,sort_keys=True)); return 0
        if args.cmd=='evaluate':
            ...
            if args.summary: Path(args.summary).write_text(summary)
            print(summary,end=''); return 0 if result['failed']==0 else 2
        if args.cmd=='promote':
            examples=_validated(args.examples); base=evaluate_predictions(examples,_predictions(args.baseline_predictions,True)); cand=evaluate_predictions(examples,_predictions(args.candidate_predictions,True)); policy=PromotionPolicy.from_dict(json.loads(Path(args.policy).read_text())); decision=compare_and_gate(base,cand,policy); payload={'baseline':base,'candidate':cand,'comparison':decision}; Path(args.out_json).write_text(json.dumps(payload,indent=2,sort_keys=True)+'\n'); md=markdown_report(decision); 
            if args.markdown: Path(args.markdown).write_text(md)
            print(md,end=''); return 0 if decision['promotion_pass'] else 3
        if args.cmd=='compare':
    ...
    except (ValueError,TrainingDependencyError,TrainingEnvironmentError) as exc: p.exit(2,f'trudy-training: {exc}\n')

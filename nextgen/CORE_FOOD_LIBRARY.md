...
The core layer targets **10,000 foods** and is designed to rank ahead of the long-tail USDA reference library in normal Nutrition searches.
...
A food can enter the 10,000-food core candidate pool only when these common values are known:
...
If fewer than 10,000 strict-tier foods exist in the available USDA releases, the remaining slots are filled using the highest-coverage candidates. Missing nutrients stay unknown.
...
The 10,000 generated Project Superhuman core foods are materialized as independent local records with stable `core:usda:<fdcId>` identifiers.
